...
function flaps_up_handler(phase)
	if 0 == phase then
		if get(external_view) == 0 then if sasl.getXPVersion() < 12 then playSample(flaps_sound, false) end end
	end
	return 0
end
...
if MASTER then
	...
	if flaps_lever_last ~= flap_lever_pos and (flap_lever_pos == 0 or flap_lever_pos == 15 or flap_lever_pos == 28 or flap_lever_pos == 36 or flap_lever_pos == 45) then
		if sasl.getXPVersion() < 12 then playSample(flaps_sound, false) end
	end
